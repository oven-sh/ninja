...
| Revert of upstream `0b115d8` "Load depfile only when output nodes are not dirty" (ninja-build/ninja#2680, on master since 2026-07, in no release). | With it, an edge that is dirty through a manifest input has its recorded deps left unloaded; when that manifest input is a `restat` output that turns out unchanged, the edge is cleaned although a depfile-only input changed, and it is rebuilt one invocation late. In Bun's graph a crate's build-script result is such an input, so editing a source file could leave the crate stale while the build reported success. `misc/output_test.py` `test_restat_clean_input_does_not_hide_depfile_input` guards it. | Not yet reported upstream. Drop the revert when upstream fixes it. |
